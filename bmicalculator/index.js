...
form.addEventListener('submit', (e)=>{
    ...
    //if anything is put on string format in the text boxes it will be converted to an integer using parseInt method.
    const height=parseInt(document.querySelector('#height').value) 
    const weight=parseInt(document.querySelector('#weight').value)
    const result=document.querySelector('#results') //gonna render the final result in dic with id results
    ...
    if(height==''||height<=0||isNaN(height)){
        ...
        //final result
        const bmi=(weight / ((height * height) / 10000)).toFixed(2) //logic to calculate
        result.innerHTML=`${bmi}` //show the result
    }
